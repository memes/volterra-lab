# Intitialise the Volterra provider using API cert/key
provider "volterra" {
  api_cert = var.volterra_cert
  api_key  = var.volterra_key
  url      = format("https://%s.console.ves.volterra.io/api", var.volterra_tenant)
}

# This module permits use of service account impersonation to allow organization
# user accounts to act as the org-wide or per-project Terraform GCP service
# account. This file defines the initialisation of google providers that are
# acting as the named service account, assuming that the invoking user account
# has the permissions to create an authentication token for the service account.
